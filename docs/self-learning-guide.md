...
## 6. 컴파일러를 소유하기 — `akela.json`

여기부터가 RFC 0002입니다 — v0.8.0부터는 추출된 엔진
[Akela](https://github.com/TimothyHan/akela)를 통해 제공됩니다
([RFC 0003](rfc/0003-akela-adoption.md)). 엔진 설정은 **`akela.json`**에 삽니다:
첫 실행 시 `.qabuddy.json` + 배포된 qa 도메인 팩으로부터 생성되고(명시적으로는
`node $QAB akela-init`), 그때부터 **당신 것**입니다 — 경로가 `~/` 이식 가능
형식이니 커밋하세요; PR로 리뷰할 수 있고 QABuddy 업데이트에도 살아남습니다.
배포 파일을 고치는 것과의 차이가 바로 이것입니다. `.qabuddy.json`은 워크플로우
설정만 유지합니다. 아래 세 능력은 전부 **opt-in**이며, 설정하지 않으면 아무
동작도 바뀌지 않습니다.
...
### 6.2 프로젝트 레퍼런스 섹션 — `PRJ` 지식 루트

팀의 방법론 파일을 배포 레퍼런스와 똑같은 자격으로 컴파일 대상에 넣습니다:

```jsonc
// akela.json — 네임스페이스당 루트 1개 (그래서 PRJ- id가 항상 명확합니다)
{ "knowledge": [
...
- 깨진 파일(태그 없는 `##`, 중복 ID)은 파일:줄을 명시하며 컴파일이 거부됩니다;
  존재하지 않는 지식 디렉터리도 루트 이름을 명시하며 거부됩니다 — 설정 오류는
  결코 어물쩍 넘어가지 않습니다.
...
### 6.5 마지막 조각 E — 그리고 당신에게 남는 수작업

RFC 0002에서 아직 안 만든 것은 **E(자동 상태 변경)** 하나입니다. E가 생기면
지문으로 반증이 확정된 학습의 은퇴와, 증거 문턱(applied ≥ 8, 모순 0)을 넘은
학습의 승격을 distill의 승인 없이 자동 적용합니다 — 감사 이벤트를 남기고,
레퍼런스 편집은 그때도 절대 자동이 아닙니다.

**E 없이 지금 당신이 손으로 하는 일의 전부:**

| 수작업 | 빈도 | E가 없애주나? |
|---|---|---|
| distill 세션 — 스킬이 제안하면 실행하고, 계획표를 읽고 승인 | 스프린트당 한 번쯤 | **일부만** — 기계적으로 결정된 행(지문 은퇴, 문턱 승격)만 미리 체크됨. 병합·복사 판정·일반화 판단은 E가 있어도 사람+LLM 몫 |
| 게이트가 열렸을 때 휴면 섹션의 원인 분류 | 게이트 열릴 때 한 번 | 아니오 — 설계상 사람 몫 (결정 6) |
| `akela.json` 편집 (오버라이드·하우스 파일·점수화) | 결정할 때만 | 아니오 — 서명은 사람이 합니다 (§6 non-goal) |
| 하우스 레퍼런스 파일 작성 | 팀 방법론이 생길 때 | 아니오 — 당신 팀만 쓸 수 있는 내용 |
| 승격 시 레퍼런스(정본) 편집 승인 | 승격마다 | 아니오 — 영원히 사람 몫 |

즉 **E가 있어도 distill 세션 자체는 남습니다** — E는 그 안건에서 몇 줄을 미리
처리할 뿐입니다. 지금은 그 기계적 행이 세션당 한두 개라 자동화가 위험만큼의
값을 못 합니다. **만들 때가 온 신호:** distill 계획표가 도장만 찍는 기계적
행으로 가득 차기 시작하면 — 그때 로그가 E를 정당화하고, 열린 설계 질문
(candidate 상태 도입 여부, 전환 시점)의 답도 데이터가 줍니다.

---

### 6.6 스킬 자체를 채점하기 — RFC 0005

위의 모든 것은 *지식*을 측정합니다: 어떤 섹션이 컴파일되고, 인용되고, 반증되었는지. 스킬의 출력이 좋아졌는지 나빠졌는지는 말해 주지 못합니다. RFC 0005가 그 계기를 더합니다: `node bin/eval.js run <skill>`은 `tests/cases/` 아래 케이스에 대해 스킬을 대상 모델에서 헤드리스로 실행하고, 별도의 Opus 판정자가 스킬 자신의 번호 매긴 제약 조건(`tests/rubric.json`)에 대해 산출물을 채점하며, 나머지는 결정적 검사가 맡고, 반드시 실패해야 하는 컨트롤을 먼저 판정합니다. `eval.js ab --a <ref> --b <ref>`는 두 QABuddy ref를 기준별로 비교합니다 — 플레이북 ablation, `/qa-improve` 회귀, 모델 업그레이드를 결정하는 벤치입니다. 루브릭은 사람이 채점한 산출물로 캘리브레이션된 뒤에만 게이트가 됩니다. 상세: [RFC 0005](rfc/0005-rubric-scored-evals.md).
