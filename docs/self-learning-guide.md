...
점수화(D)와 자동 상태 변경(E)은 코드로는 아직 없습니다. 생기면 기본 off이며,
켜는 길은 두 가지입니다: **게이트를 당신 데이터로 통과하거나**, 메인테이너가
**명시적 오버라이드를 로그에 결정으로 기록하며** 켜는 것 (RFC 0002 §2.4).
막혀 있는 것은 점수화 자체가 아니라 *조용히* 켜지는 것입니다.
