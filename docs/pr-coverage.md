...
- **버그**는 동반 PR 설명과 알림에 "고칠 것"으로 오르고, 해당 조건에 ⚠️가 붙습니다.
  작성자가 원래 브랜치에서 고친 뒤 `/qabuddy heatmap`으로 확인합니다. 버그 파일의
  `**Status:**` 줄이 목록에 남을지를 정합니다: `fixed`, `verified`, `wont-fix`, `duplicate`는
  "Resolved" 아래로 내려갑니다.
- **이전 실행의 발견**은 다음 탐색이 다시 확인하고 상태와 함께 다시 나열합니다; `resolved`나
  `not reproduced`는 목록에서 빼고 그 발견이 열었던 이슈를 닫습니다. 다시 나타난 발견은 두
  번째 이슈를 여는 대신 기존 이슈를 다시 엽니다.
