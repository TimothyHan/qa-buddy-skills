...
| | |
|---|---|
| **상태** | PR 트리거 워크플로우 절반은 **Withdrawn**(2026-09-25): 0.9.0에 실험적으로 배포, 0.10.0에서 너무 이르다는 판단으로 제거 — 데모 저장소 하나는 다른 팀의 풀 리퀘스트에 봇을 올릴 근거로 부족했다. `v0.9.0` / `v0.9.1`을 고정한 호출자는 계속 동작한다. 헤드리스 모드 절반(§2, `--headless`, `claude -p` 러너)은 유효하며 RFC 0005가 그 위에서 돈다. |
