...
| 16 | A/B gate | **relative to A**: a regression outside the spread or a floor breached only in B blocks; the absolute verdict against the threshold is information. `ab` skips when `scope` finds no criterion citing the change (2026-09-12, after the first real run on #87) | `main` itself scored 0.571 against 0.857: an absolute gate fails every PR and says nothing about the change; and three runs cannot measure a change no criterion grades — $22 to learn that once is enough |
...
### First real gate run (2026-09-12, PR #87)

`eval.js ab test-cases --a main --b improve/mapping-json-contract`, 18 runs, ≈ $22, three hours with one crash. Both sides mean 0.571 against 0.857; verdict FAIL on both; no delta outside the spread. The change under test — one Phase 4 bullet — touched nothing any criterion cited, so the run could not have measured it. What the run taught, and what changed:

- **thin-ticket was bimodal (0.00 or 0.86).** Four runs of six wrote no file at all: the skill closed NEEDS_CONTEXT in chat, and the judge scored absence as 0 on every criterion — correctly. The calibration artifact for that case (2026-09-05, scored acceptable at 3/3/3/3/3) had written its document and mapping. Fixed in the skill, not the judge: constraint 8, *a close is a file, not a message*; `status-block` now cites it; the judge notes say a zero-case document naming both placeholder ACs is the full-marks answer.
- **The baseline cannot pass its own threshold.** With thin-ticket removed from the picture, `coverage-honesty` 1.22 and `probed-app` 1.33 on `main` remain: the skill claims coverage it did not verify and skips the app probe in headless mode (the same finding as ablation 1). Filed as the next `/qa-improve` item; until it lands the absolute verdict on test-cases reads FAIL, which is why decision 16 makes the gate relative.
- **Deterministic properties are `check` criteria.** The property PR #87 fixed (every mapping parses) is now `artifacts-parse` (`json_valid`, floor 1) with a broken-comma control; it would have failed on the bench before it failed in production.
- **The harness was fragile.** A full disk killed the run after eight of nine B runs; the symlink restore stopped at the first error and left every QABuddy skill on the machine dangling; the workspaces were in temp and gone. Now: restore on signals and in a resilient `finally`, `ab --restore` from the lock's snapshot, variants under the A/B's own directory, per-run `scores.json` and `ab --resume`.
- **Recalibrated after the rubric change (2026-09-12, $1.95 judge-only + $1.95 written).** Same ten entries, same human scores, judge Opus: agreement traceability 1.0, coverage-honesty 1.0, dedup-by-assertion 1.0, prioritization 1.0, observed-or-unverified 0.93; floor agreement 1.0; pair agreement 0.95; **threshold 0.867** (the added check criterion shifts the weighted totals; the human/judge agreement is unchanged, so the baseline's 0.571 is the skill, not the bar).
- **Verification A/B on the fix (thin-ticket only, 3 + 3 runs, $3.46):** A `main` 0.867 / 0 / 0.867 — the bimodality reproduced, and the new runner line under the zero run reads "NEEDS_CONTEXT — no test cases could be generated" with no file written; B 0.867 / 0.867 / 0.867, every run a zero-case document with `## Blocker`, the status block and both ACs under `unmappedACs`. Gate PASS; `probed-app` breaches on both sides because the case has no app and the process check has no notion of "not applicable" — a rubric item for the next pass.
- **Run-id keying** (`activity-<task|branch>-hash`, the engine's marker file shared by parallel agents in one checkout) is an Akela upstream item, not an eval one — the eval gives every run its own workspace.
