 ...
 *            [--logs DIR] [--billing subscription|api-key]   (footer: model spend per phase and who pays)
...
function renderMarkdown(h) {
  ...
  if (h.spend) {
    const parts = Object.entries(h.spend.phases).map(([k, v]) => `${k} $${v.toFixed(2)}`);
    const who = h.spend.billing === 'subscription' ? 'billed to the Claude subscription that minted `CLAUDE_CODE_OAUTH_TOKEN`'
      : h.spend.billing === 'api-key' ? 'billed to `ANTHROPIC_API_KEY` (API credit)' : 'billing source unknown';
    L.push(`💳 Model spend this run: ${parts.length ? parts.join(' · ') + ' = ' : ''}**$${h.spend.total.toFixed(2)}** — ${who}`, '');
  }
  L.push(`<sub>Generated by QABuddy pr-coverage ${h.version} at ${h.generatedAt}</sub>`);
  return L.join('\n') + '\n';
}

// Model spend of this run, from the per-phase execution logs (claude-<phase>.json), and who pays.
function readSpend(dir, billing) {
  if (!dir && !billing) return null;
  const phases = {}; let total = 0;
  if (dir && fs.existsSync(dir)) for (const f of listDir(dir).sort()) {
    const m = /^claude-(\w+)\.json$/.exec(f); if (!m) continue;
    const a = readJson(path.join(dir, f), null); if (!a) continue;
    const r = (Array.isArray(a) ? a : [a]).find(x => x && x.type === 'result') || (a.total_cost_usd !== undefined ? a : null);
    if (!r) continue;
    const c = +(r.total_cost_usd || 0); phases[m[1]] = c; total += c;
  }
  return { phases, total: Math.round(total * 100) / 100, billing: billing || 'unknown' };
}

function cmdHeatmap(o) {
  if (!o.touched) usage('heatmap needs --touched');
  const h = buildHeatmap(o);
  const spend = readSpend(o.logs, o.billing);
  if (spend) h.spend = spend;
...
function cmdPreflight(o) {
  ...
  if (o.md) {
    ...
    if (billing) L.push('', `💳 Model spend on this PR is billed to ${billing}.`);
...
const REUSABLE_WORKFLOW = 'TimothyHan/qa-buddy-skills/.github/workflows/qa-buddy-pr.yml';
const LABELS = [
...
on:
  pull_request:
    types: [opened, ready_for_review, labeled, closed]   # closed: a merged companion PR refreshes the heatmap (or chains, see after-companion-merge)
  issue_comment:
    types: [created]
...
jobs:
  qabuddy:
    ...
    with:
      ...
      # delivery: companion-pr      # companion-pr (reviewable) | commit (push onto the PR's branch)
      # default-phases: kb          # on PR open: kb | kb,explore | kb,automate | kb,explore,automate
      # after-companion-merge: none   # none: a merged companion only refreshes the heatmap | full | automate — chain more phases after review
...
function cmdInit(o) {
  ...
  const next = [];
  next.push('set one secret — this decides who pays: `claude setup-token` then `gh secret set CLAUDE_CODE_OAUTH_TOKEN` bills the Claude subscription of whoever minted it (a team repo should use a dedicated account), or `gh secret set ANTHROPIC_API_KEY` bills API credit; verify with `gh secret list`');
  ...
  process.stdout.write(JSON.stringify({ schema: 'pr-init/1', workflow: path.relative(root, wf).split(path.sep).join("/"), reusable: REUSABLE_WORKFLOW, ref: o.qabuddyRef || 'v0.9.1', labels, next }, null, 2) + '\n');
}
