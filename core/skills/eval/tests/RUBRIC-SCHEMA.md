...
| gate | rule |
|---|---|
| (b) agreement | exact-score agreement ≥ 0.8 per judge criterion over every (entry, pass) with a human score; floor agreement 1.0 on floored criteria, decided **per entry on the majority of passes** (three passes exist so one noisy pass is outvoted; the per-pass figure is reported alongside) |
