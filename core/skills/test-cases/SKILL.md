...
8. Every JSON file you wrote parses: run `node -e 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"))' <path>` on each mapping and fix it before saving. A mapping that does not parse hides its test cases from every reader — the eval bench, the KB tooling, a future coverage scan — until repaired (caught live: a missing comma written in headless mode)
