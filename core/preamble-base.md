...
2. **Cite and log:** when a source shapes output — a learning (`LRN-…`) or a reference section (`REF-<file-stem>#<id>`, the id is in the `<!-- qab: id=… -->` comment under the heading; `REF-playbook/<stem>#<id>` under playbook/) — cite its ID and run `node $QAB log applied <ID>` (once per source per run; the helper rejects unknown REF ids and suggests the nearest). If live observation contradicts an active source, do not apply it — run `node $QAB log contradicted <ID> --note "<what you saw>"` and flag it in the report. Anything noteworthy mid-run goes to `## Candidate learnings` in the run's `scratchpad.md` — no evidence bar.
