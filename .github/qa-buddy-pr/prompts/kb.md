...
(`testCases[{id, layer, type, status}]`); keep existing TC ids, and number new ones after the highest `TC-NNN` already used anywhere in the feature's `test-cases/` — ids are unique per feature, never per ticket.
