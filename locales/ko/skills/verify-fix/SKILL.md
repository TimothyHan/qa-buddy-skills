...
원인이 실패한 파이프라인·배포 단계(CI red, 빌드 깨짐, 배포 잡 실패)라면 먼저 지문을
남깁니다: `node $QAB fp ci-step-failed "<pipeline>/<step>"` -- 이 클래스를 막는다고
주장한 학습이 있으면 `active` 아래 나타납니다; 플래그하세요.
...
**재현되지 않는다면 그것은 아직 판정이 아닙니다.** 재현 단계는 무엇을 했는지를 적을 뿐,
그때 함께 참이었던 조건은 적지 않습니다. 결론을 내리기 전에 최초 실행의 조건을 재구성하고
다시 시도하세요 — `{{REFERENCE_PATH}}/playbook/defect-lifecycle.md` §재현되지 않을 때
(시각·타임존, 로케일, 뷰포트, 계정, 데이터 상태, 브라우저 세션, 빌드).
...
5. 재현되지 않았다면, 재구성한 실행 조건을 **이름을 들어 나열**했고 판정이 VERIFIED가 아니라 NOT REPRODUCED인지
6. **형식 확인:** 보고서에 판정, 재현 테이블, 회귀 테이블, 다음 단계가 포함되어 있는지
...
| **NOT REPRODUCED** | 원래 버그가 더 이상 보이지 않지만 그것을 설명하는 수정이 없거나 조건을 되살리지 못함 | 버그는 열린 채로 두고 "not reproduced under: {시도한 조건}" 코멘트. 조용히 VERIFIED 금지 |
...
## 판정: {VERIFIED | FAILED | REGRESSION | NOT REPRODUCED}
{한 줄 요약}
