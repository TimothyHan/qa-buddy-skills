...
**이전 발견부터.** `features-kb/features/{KEY}/exploratory/`에 이전 세션이 있으면, 새 탐색 전에 그 발견들을 각각 다시 확인하고 이 보고서에 **같은 제목**으로 다시 나열하되 `**상태:**`를 `still open`, `resolved`, `not reproduced` 중 하나로 적습니다(실제 일어난 것 아래 한 줄로 본 것을). PR 파이프라인은 세션을 가로질러 기능+제목으로 발견을 식별합니다: 다시 나열하지 않은 이전 발견은 영원히 열려 있고, 다시 나열한 발견은 당신의 상태를 따릅니다.
...
```markdown
### 발견 {N}: {제목}
**범주:** {범주} | **심각도:** {레벨} | **우선순위:** {레벨} | **상태:** {open | still open | resolved | not reproduced}
**초점 영역:** {영역} | **발견 경로:** {휴리스틱}
