...
8. 작성한 모든 JSON 파일이 파싱되는지: 매핑마다 `node -e 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"))' <경로>`를 실행하고 저장 전에 고칩니다. 파싱되지 않는 매핑은 고칠 때까지 모든 리더 — eval 벤치, KB 도구, 미래의 커버리지 스캔 — 에게서 그 테스트 케이스를 감춥니다(실전 확인: 헤드리스 모드에서 쉼표 하나 빠짐)
