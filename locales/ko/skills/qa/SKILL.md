...
description: |
  ...
  사용하지 않는 경우: QA 방법론 질문, 버그 수정 검증(/qa-verify-fix 사용), 버그 등록 없이 탐색만 할 때(exploratory).
tool-groups:
  - bash
  - read
  - write
...
7. **한 번에 하나의 티켓만 처리합니다.** 여러 티켓을 테스트하려면 티켓별로 `/qa-qa`를 실행하세요.
8. **스프린트에 연결하세요.** Jira와 KB를 업데이트하세요 — 로컬 마크다운에만 남긴 결과는 팀에서 보이지 않습니다.
...
| *(공통)* | **재시도하면 재현되지 않는 결과는 PASS가 아님.** 기록 전에 실행 조건을 되살리고(`{{REFERENCE_PATH}}/playbook/defect-lifecycle.md` §재현되지 않을 때: 시각·타임존, 로케일, 뷰포트, 계정, 데이터 상태) 시도한 조건을 노트에 남길 것 |

결과를 기록하면서 실패 클래스를 지문으로 남깁니다 (테스트 케이스당이 아니라 실행당
서로 다른 클래스마다 한 줄): 실제 결과가 기대와 모순되는 FAIL →
`node $QAB fp assertion-mismatch "<TICKET>/<TC id>"`; 앱이나 의존성에 접속할 수 없어
BLOCKED → `node $QAB fp env-unreachable "<url 또는 서비스>"`; 로그인·세션 만료로
BLOCKED → `node $QAB fp auth-failed "<url 또는 계정 역할>"`. 헬퍼가 `active` 아래
학습을 나열하면 그 규칙이 이 클래스를 막지 못한 것입니다 -- 보고서에 플래그하세요.
...
- **NOT TESTED** — 이 AC에 대한 테스트 케이스가 없음 (커버리지 갭으로 표시); 재발을
  셀 수 있도록 `node $QAB fp ac-unmapped "<TICKET>/AC<n>"`도 실행
...
3. QA 보고서를 KB에 저장하여 이후 실행과 팀원이 찾을 수 있도록 합니다
