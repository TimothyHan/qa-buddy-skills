...
어느 실행 게이트든 빨간불이면 `test-results/` 아래 실패한 테스트의 `error-context.md`와 트레이스만 읽고, 전체 실행 출력은 읽지 않습니다 — 요약 줄이 게이트의 판정입니다.
...
게이트가 잡아낸 것을 지문으로 남깁니다 (서로 다른 실패마다 한 줄, 수정 시도 후·보고
전): 게이트 2/3에서만 실패하는 테스트 → `node $QAB fp spec-flaky "<스펙 파일> › <TC id>"`;
스펙이 필요로 했지만 얻지 못한 fixture·storageState·시딩된 사전 조건 →
`node $QAB fp fixture-missing "<fixture 또는 사전 조건>"`. 헬퍼가 `active` 아래 학습을
나열하면 그 규칙이 이를 막지 못한 것입니다 -- 플래그하세요.
