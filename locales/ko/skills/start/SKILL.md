...
1. **매 단계 후 멈춘다.** SDT의 승인 없이 다음 단계로 넘어가지 않는다. 헤드리스: 일시정지는 질문이 아니라 스크래치패드의 `## State` 한 줄이다 -- 계속 진행한다.
