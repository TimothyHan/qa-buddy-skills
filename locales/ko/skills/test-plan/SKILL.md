...
2. **작성 전에 요약부터 보여주세요.** SDT에게 컨텍스트를 확인받지 않고 전체 계획을 바로 작성하지 마세요. 헤드리스: 요약을 스크래치패드에 쓰고 계속 진행합니다.
...
4. **AC를 추출하고 `feature.md`를 초기화합니다 — 계획 작성 전에 먼저 수행하세요.**
   ...
   이어서 `features-kb/features/{EPIC-KEY}/sources.json`(KB 명세 §6.8)을 씁니다: 이 기능이 소유한 코드 glob과 단위/API/e2e 테스트 위치 -- 티켓, `feature.md`의 **Source**, `git log`에서 도출합니다. 대화형: 제안 후 확인. 헤드리스: 필수.
...
8. **SDT에게 결과를 요약합니다.** "계획을 작성하기 전에 빠진 내용이나 잘못된 부분이 있나요?"라고 질문합니다. (헤드리스: 질문 생략)
...
1. **SDT에게 초안을 제시합니다** (헤드리스: 파일을 쓰는 것이 곧 제시)
2. **피드백에 따라 수정합니다** (헤드리스: 생략)
...
5. **Confluence에 게시합니다** (SDT에게 어떤 스페이스/상위 페이지에 올릴지 확인; 헤드리스이거나 Confluence 도구가 없으면 생략)
