...
The engine is [Akela](https://github.com/TimothyHan/akela) (npm `akela`, pinned exactly in `package.json`, vendored code-only by the build into `dist/<platform>/references/engine/akela/`). `bin/akela.js` is QABuddy's launcher — `QAB_*`→`AKELA_*` env mapping, first-run `akela.json` generation from `.qabuddy.json` + the qa domain pack, in-process delegation — and `bin/qab.js` is its deprecation shim (one release). The engine is the only writer of `learnings-log.jsonl` and `fingerprints.jsonl` — the model passes bare arguments and never hand-writes JSON. Schema: `self-improve.md` §Learnings log and §Failure fingerprints (`"v": 1`; readers accept every earlier version — logs are append-only and live in users' repos for years; new lines are `activity`-keyed, historical `skill` lines still read). The fingerprint `kind` vocabulary is closed and lives in the **qa domain pack** (`core/engine/qa.domain.json`, mirrored in `self-improve.md`; `test.js` checks they match) — add a kind only with a detection point in a skill and its ko twin. `akela.js scoreboard` writes `features-kb/.cache/scoreboard.json`, a derived cache — never read it as truth, never commit it. Runtime files (`LEARNINGS.md`, `learnings-log.jsonl`, `fingerprints.jsonl`, `.qa-reports/`) are project content: never in this repo, never dual-locale. **Changing the engine** (scope resolution, packing, the score formula or its floor, the gate) now happens **upstream in Akela**: PR there, release, bump the pin here — the scoring shape stays fixed by RFC 0002 decisions 10–13 (per-profile with a floor, constants not knobs — never a global ranking), which Akela's code cites. QABuddy-side compiler surface is the pack, the launcher, and the config contract. Every engine-touching change — pin bumps included — is gated by the **harness** (`testAkelaEquivalence` plus the full suite running through the shim): it must stay green, and behavioural changes ride only on a deliberate pin bump with the delta recorded in RFC 0003 §2 style.
