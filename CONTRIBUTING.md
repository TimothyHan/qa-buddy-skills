...
| 구성 요소 | 예산 |
|-----------|--------|
| Preamble (Tier 1) | ~102줄 |
| Preamble (Tier 2) | ~123줄 (Tier 1 + 21) |
...
**로드맵:** 학습 레이어를 산문 판단에서 측정 기반으로 옮기는 작업은 끝났습니다 — 실행별 컴파일된 지식 슬라이스, append-only 학습 로그, 산술 기반 distill, eval 게이트 승격. 그것이 [RFC 0001 — Context Compiler](docs/rfc/0001-context-compiler.md)(영문, 한국어 요약 포함)이며, v0.5.0으로 배포되고 **PR0–PR6에서 닫혔습니다**: §9.3 게이트는 열렸고, 그 게이트가 인가한 측정은 이 프로젝트의 증거에서 점수 기반 선택에 반대했습니다(결정 16). 따라서 점수 기반 선택과 자동 상태 변경은 QABuddy가 한 번 배포하는 예정된 단계가 아니라, 프로젝트가 자기 측정으로 여는 능력입니다 — [RFC 0002 — 프로젝트 소유 컴파일러 설정](docs/rfc/0002-project-owned-compiler.md)(Accepted — A–D를 v0.7.0으로 배포: 스코프 오버라이드, 프로젝트 레퍼런스, 게이트 리포트, 게이트 뒤의 점수화 + 게이트 열림 알림; E는 프로젝트의 데이터가 요구할 때까지 미구현 — 그 신호는 [사용자 가이드](docs/self-learning-guide.md) §6.5에 적혀 있습니다). 이 가이드에서 특정 단계와 함께 바뀌는 섹션은 해당 단계의 PR에서 갱신합니다 — 미리 바꾸지 않습니다.
...
- 테스트: 최초 1회 `npm ci` (동등성 하니스가 실행하는 고정 버전 `akela` 엔진
  설치 — RFC 0003), 그 다음 `node test.js`
...
| 계층 | 주입 내용 | 사용 대상 |
|------|---------|---------|
| `1` | 컨텍스트 복구 + 컨텍스트 소스 + 프로젝트 학습 + 리뷰 옵션 + 헤드리스 모드 + 완료 상태 (89줄) | 경량 스킬 |
...
| 시점 | 의무 | 기록 위치 |
|---|---|---|
| 학습이 출력을 결정할 때마다 | ID 인용; `akela.js log applied LRN-…` | `learnings-log.jsonl` |
| 실행 중 관찰이 active 학습과 모순 | 적용하지 않음; `akela.js log contradicted LRN-… --note`; 보고서에 플래그 | `learnings-log.jsonl` |
| 완료 | 세 가지 포착 트리거 적용; 발화하면 LRN 작성 + `log captured`; 그 다음 `log outcome --status <S>` | `LEARNINGS.md`, `learnings-log.jsonl` |
| 시작 | `akela.js compile --skill <name>` → `slice.md` 읽기 (학습 파일 + 매니페스트에 나열된 레퍼런스 섹션 읽기를 대체); 폴백: 레퍼런스 + `LEARNINGS.md` | `.qa-reports/runs/<run>/{slice.md,profile.json,scratchpad.md,events.jsonl}` |
...
| 이름 붙은 실패 클래스를 만남 (`e2e-pom` heal → `locator-not-found`; `e2e-write` 게이트 → `spec-flaky`, `fixture-missing`; `qa` → `ac-unmapped`, `env-unreachable`, `auth-failed`, `assertion-mismatch`; `verify-fix` → `ci-step-failed`) | `akela.js fp <kind> "<key>"` — 실행당 서로 다른 클래스마다 한 줄; 헬퍼가 `active` 아래 학습을 나열하면 플래그 | `fingerprints.jsonl` |
| 이 실행에 지문이 있는 트리거 1 포착 | 새 LRN의 `Fingerprint:`를 그 ffp로 설정 (`akela.js fp --list`) | `LEARNINGS.md` |

엔진은 [Akela](https://github.com/TimothyHan/akela)입니다 (npm `akela`, `package.json`에 정확히 고정, 빌드가 코드만 `dist/<platform>/references/engine/akela/`로 벤더링). `bin/akela.js`는 QABuddy의 런처입니다 — `QAB_*`→`AKELA_*` 환경변수 매핑, 첫 실행 시 `.qabuddy.json` + qa 도메인 팩으로부터 `akela.json` 생성, 인프로세스 위임 — 그리고 `bin/qab.js`는 지원 중단 심입니다(한 릴리스). 엔진이 `learnings-log.jsonl`과 `fingerprints.jsonl`의 유일한 작성자입니다 — 모델은 인자만 넘기고 JSON을 손으로 쓰지 않습니다. 스키마: `self-improve.md` §학습 로그·§실패 지문 (`"v": 1`; 리더는 모든 이전 버전을 수용 — 로그는 append-only이고 사용자 저장소에 수년간 남습니다; 새 라인은 `activity` 키, 과거의 `skill` 라인도 그대로 읽힙니다). 지문 `kind` 어휘는 닫혀 있고 **qa 도메인 팩**(`core/engine/qa.domain.json`, `self-improve.md`에 미러; `test.js`가 일치를 검사)에 삽니다 — 스킬에 감지 지점과 ko 대응본을 함께 넣을 때만 kind를 추가하세요. `akela.js scoreboard`는 `features-kb/.cache/scoreboard.json`(파생 캐시)을 씁니다 — 진실로 읽지도, 커밋하지도 마세요. 런타임 파일(`LEARNINGS.md`, `learnings-log.jsonl`, `fingerprints.jsonl`, `.qa-reports/`)은 프로젝트 콘텐츠입니다: 이 저장소에 없고, 이중 로케일도 아닙니다. **엔진을 바꾸는 일**(스코프 해석, 패킹, 점수 공식과 바닥값, 게이트)은 이제 **Akela 업스트림**에서 일어납니다: 거기에 PR, 릴리스, 여기서 핀 범프 — 점수화의 형태는 여전히 RFC 0002 결정 10–13(바닥값을 가진 프로파일별, 노브가 아닌 상수 — 전역 랭킹 금지)에 고정되어 있고, Akela 코드가 이를 인용합니다. QABuddy 쪽 컴파일러 표면은 팩·런처·설정 계약입니다. 엔진을 건드리는 모든 변경은 — 핀 범프 포함 — **하니스**(`testAkelaEquivalence` + 심을 거쳐 도는 전체 스위트)로 게이트됩니다: 항상 green이어야 하고, 동작 변화는 의도된 핀 범프에만 실리며 델타는 RFC 0003 §2 방식으로 기록합니다.
