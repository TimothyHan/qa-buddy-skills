...
필요한 발견은 이슈가 되며, 베이스 브랜치에는 아무것도 쓰지 않습니다. `/qa-setup`으로
설정합니다 -- 이미 구성된 저장소는 *유지하고 PR 자동화 설정*을 고르거나 `/qa-setup --pr`
-- 또는 `pr-coverage.js init` 한 번으로도 됩니다. `poc/cloud-service` 브랜치의 개념
증명이며 아직 릴리스에는 없습니다: 그 브랜치에서 빌드해야 합니다.
