...
**스크럼 팀을 위한 AI 기반 QA 파트너**

[English](README.md) · [한국어](README-ko.md)

[![License: Apache 2.0](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](LICENSE)
[![Skills: 11](https://img.shields.io/badge/Skills-11-green.svg)](#스킬)
...
[![Tests: 475](https://img.shields.io/badge/Tests-475_passing-brightgreen.svg)](#테스트)
...
| 스킬 | 명령어 | 기능 |
|-------|---------|------|
| **Improve** | `/improve` | 스킬 실패 분석, 수정 제안, 적용, eval 실행, PR 생성 |
...
| **(A) 승인** | 다음 단계로 진행 |
| **(B) 내용 피드백** | 결과물을 반복 수정 |
| **(C) 도구 피드백** | AI가 스킬 소스를 읽고 수정을 제안, 빌드, eval 실행 후 재개 |
...
QABuddy는 실제 사용을 통해 스스로를 개선합니다. 네 가지 방식:

**1. 모든 리뷰 지점에서 인라인으로** — 모든 중단 지점에서 **(C) 도구 피드백**을 선택할 수 있습니다. AI가 스킬 소스를 읽고 수정을 제안하고, 빌드, eval 실행 후 재개합니다.

**2. `/improve`** — 상세한 실패 분석을 위한 전용 스킬. 구조화된 제안을 생성하고, 수정을 적용한 후 eval 픽스처를 실행하고, PR을 생성합니다.

**3. `/eval`** — 모든 스킬에 대해 eval 픽스처를 실행합니다. 11개 스킬에 걸쳐 43개 픽스처와 7개 어설션 연산자 (`contains`, `not_contains`, `matches`, `exists`, `eq`, `length_eq`, `length_gte`).

**4. 업스트림 기여** — `/qa-setup`에서 선택할 수 있습니다. `/improve`가 스킬을 수정하면, 수정 사항이 자동으로 QABuddy 저장소에 PR로 제출되어 모든 사용자에게 도움이 됩니다.
